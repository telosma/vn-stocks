...
function getStockdata(symbols) {
  const response = UrlFetchApp.fetch(
    'https://priceservice.vndirect.com.vn/priceservice/secinfo/snapshot/q=codes:' +
      symbols
  )
  const json = response.getContentText()
  const data = JSON.parse(json)

  const stockData = data.reduce((accumulator, d) => {
...
function setDataGoogleSheet() {
    ...
    if (oldPrice - curPrice != 0) {
      if (curPrice >= boughtPrice) {
        ...
        let totalProfit = parseFloat(
          profit * currentStock * PRICE_UNIT
        ).toFixed(0)
        message += `<p>Code: ${makeProfitFontTag(
          code
        )} | Profit(%): ${makeProfitFontTag(
          profitPercent
        )} | Hold Stocks: ${currentStock} | Total Profit: ${makeProfitFontTag(
          totalProfit
        )} ${PRICE_CURRENCY}</p>`
      } else {
        ...
        message += `<p>Code: ${makeLossFontTag(
          code
        )} | Loss(%): ${makeLossFontTag(
          lossPercent
        )} | Hold Stocks: ${currentStock} | Total Loss: ${makeLossFontTag(
          totalLoss
        )} ${PRICE_CURRENCY}</p>`
      }
    }
  ...
  sheet
    ...
    .setValue(
      'Last update: ' +
        new Date().toLocaleString('vn-VI', { timeZone: 'Asia/Ho_Chi_Minh' })
    )
  return message
}
...
function sendMail(htmlBody) {
  for (var i in Config.email_receiver) {
    MailApp.sendEmail(Config.email_receiver[i], MAIL_SUBJECT, '', {
      htmlBody: htmlBody,
    })
  }
}
